...
EncFS_Context::EncFS_Context():openFiles(MAX_FILE_CACHE_SIZE)
{ 
...
int EncFS_Context::openFileCount() const
{
    ...
    return static_cast<int>(openFiles.size());
}
...
shared_ptr<FileNode> EncFS_Context::lookupNode(const char *path)
{
    ...
    // Look for key in File Cache and return it if it exist otherwise Create a new FileNode and return.
	if (openFiles.containsKey( std::string(path)))
	{
		//return openFiles.get( std::string(path) )->node;
		return openFiles.get( std::string(path) );
	}
    else
    {
...
#endif
    if ( !openFiles.replaceKey( std::string(from), std::string(to) ))
    { 
        // POC TODO file is not in open file cache
        std::cout<<" Renamed node not found in Cache "<<std::endl;
    }
}
	...
	const boost::shared_ptr<FileNode> &node)
{
    ...
    //openFiles.put( std::string(path), pl);
    std::cout<<"    use_count of node "<<node.use_count()<<endl;
    openFiles.put( std::string(path), node);
    std::cout<<"    use_count of node "<<node.use_count()<<endl;
    // POC TODO Need to remove this Placeholder in heap. Return value is used by caller
    return (void *)pl;
}
...
{
    Lock lock( contextMutex );
    openFiles.remove(std::string(path));
#if 0
