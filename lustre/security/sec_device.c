...
int sec_maxeasize_get(const struct lu_env *env, struct md_device *md,
		      int *easize)
{
	struct sec_device *sec_dev = md2sec_dev(md);

	return sec_dev->sec_child->md_ops->mdo_maxeasize_get(env,
						    sec_dev->sec_child,
						    easize);
}
		 ...
		 struct lu_fid *fid)
{
	...
	return sec_dev->sec_child->md_ops->mdo_root_get(env,
						    sec_dev->sec_child,
						    fid);
}
		      ...
		      struct obd_statfs *sfs)
{
	...
	ENTRY;
	rc = sec_dev->sec_child->md_ops->mdo_statfs(env,
			      ...
			      struct lustre_capa_key *keys)
{
	...
	rc = sec_dev->sec_child->md_ops->mdo_init_capa_ctxt(env,
							sec_dev->sec_child,
							mode, timeout, alg,
							keys);
	RETURN(rc);
			       ...
			       struct lustre_capa_key *key)
{
	...
	int rc;
	ENTRY;
	rc = sec_dev->sec_child->md_ops->mdo_update_capa_key(env,
							 sec_dev->
							 sec_child, key);
	RETURN(rc);
}
			     ...
			     int idx, void **h)
{
	...
	rc = sec_dev->sec_child->md_ops->mdo_llog_ctxt_get(env,
						       sec_dev->sec_child,
						       idx, h);
	RETURN(rc);
}
...
static int sec_quota_notify(const struct lu_env *env, struct md_device *m)
{
	...
	rc = sec_dev->sec_child->md_ops->mdo_quota.mqo_notify(env,
							  sec_dev->
							  sec_child);
	RETURN(rc);
}
			   ...
			   void *data)
{
	...
	rc = sec_dev->sec_child->md_ops->mdo_quota.mqo_setup(env,
							 sec_dev->
							 sec_child, data);
	RETURN(rc);
}
...
static int sec_quota_cleanup(const struct lu_env *env, struct md_device *m)
{
	...
	rc = sec_dev->sec_child->md_ops->mdo_quota.mqo_cleanup(env,
							   sec_dev->
							   sec_child);
	RETURN(rc);
}
			      ...
			      struct md_device *m)
{
	...
	rc = sec_dev->sec_child->md_ops->mdo_quota.mqo_recovery(env,
							    sec_dev->
							    sec_child);
	RETURN(rc);
}
			   ...
			   __u32 type)
{
	...
	rc = sec_dev->sec_child->md_ops->mdo_quota.mqo_check(env,
							 sec_dev->
							 sec_child, type);
	RETURN(rc);
}
			...
			__u32 type)
{
	...
	rc = sec_dev->sec_child->md_ops->mdo_quota.mqo_on(env,
						      sec_dev->sec_child,
						      type);
	RETURN(rc);
}
			 ...
			 __u32 type)
{
	...
	rc = sec_dev->sec_child->md_ops->mdo_quota.mqo_off(env,
						       sec_dev->sec_child,
						       type);
	RETURN(rc);
}
			     ...
			     struct obd_dqinfo *dqinfo)
{
	...
	rc = sec_dev->sec_child->md_ops->mdo_quota.mqo_setinfo(env,
							   sec_dev->
							   sec_child, type,
							   id, dqinfo);
	RETURN(rc);
			     ...
			     struct obd_dqinfo *dqinfo)
{
	...
	rc = sec_dev->sec_child->md_ops->mdo_quota.mqo_getinfo(env,
							   sec_dev->
							   sec_child, type,
							   id, dqinfo);
	RETURN(rc);
			      ...
			      struct obd_dqblk *dqblk)
{
	...
	rc = sec_dev->sec_child->md_ops->mdo_quota.mqo_setquota(env,
							    sec_dev->
							    sec_child,
							    type, id,
							    dqblk);
			      ...
			      struct obd_dqblk *dqblk)
{
	...
	rc = sec_dev->sec_child->md_ops->mdo_quota.mqo_getquota(env,
							    sec_dev->
							    sec_child,
							    type, id,
							    dqblk);
			      ...
			      struct obd_dqinfo *dqinfo)
{
	...
	rc = sec_dev->sec_child->md_ops->mdo_quota.mqo_getoinfo(env,
							    sec_dev->
							    sec_child,
							    type, id,
							    dqinfo);
			      ...
			      struct lu_device *d, struct lustre_cfg *cfg)
{
	...
	/* TODO: Add cases here */
	case LCFG_CLEANUP:
		lu_site_purge(env, sec2lu_dev(m)->ld_site, ~0);
		if (m->sec_child_exp)
...
static int sec_init0(const struct lu_env *env, struct sec_device *sec,
		     struct lu_device_type *t, struct lustre_cfg *lcfg)
{
	int rc;
	ENTRY;

	sec->sec_md_dev.md_lu_dev.ld_ops = &sec_lu_ops;
	sec->sec_md_dev.md_ops = &sec_md_ops;

	rc = sec_connect_to_next(env, sec, lustre_cfg_string(lcfg, 3));
	if (rc)
		RETURN(rc);

//	dt_conf_get(env, sec->sec_child, &sec->sec_dt_conf);
...
static int __init sec_mod_init(void)
{
	...
	return class_register_type(&sec_obd_device_ops, NULL, NULL,
