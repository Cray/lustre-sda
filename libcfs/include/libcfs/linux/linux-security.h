...
#define CFS_SID_CACHE_SIZE 512
#define CFS_SID_MAX_LEN 128

typedef struct hlist_head cfs_sid_cache_t[CFS_SID_CACHE_SIZE];
...
int cfs_security_unlink(__u32 tsid, __u32 dsid, __u32 isid, umode_t mode,
			char *name);
...
int cfs_security_setxattr(__u32 tsid, __u32 sbsid, __u32 isid, umode_t mode,
			  char *fname, const void *value, size_t size,
			  char *name, __u64 capa, __u32 fsuid, __u32 iuid);
int cfs_security_removexattr(__u32 tsid, __u32 isid, umode_t mode, char *fname,
			     char *name, __u64 capa);
...
int cfs_security_lock(__u32 tsid, __u32 osid, __u32 fsid, umode_t mode,
		      char *name);

int cfs_security_init_object(__u32 msid, __u32 dsid, __u32 tsid, __u32 tcsid,
			     umode_t mode, char **name, void **value,
			     size_t *len);
