...
public:
    ...
    // unknown)
    std::string nextPlaintextName(const CipherKey &key, bool bChainedNameIV, int *fileType=0, ino_t *inode=0);
...
public:
    ...
    */
    shared_ptr<FileNode> openNode( const char *plaintextName, 
     ...
     /*
      The reentrant version of the openNode(), reentrant is safe for same thread....
     */
    shared_ptr<FileNode> _openNode_safe_reentrant( const char *plaintextName, 
            const char *requestor, int flags, int *openResult );
    ...
    */
    ...
    CipherKey getFileKey(const char *plaintextPath, bool isDirectory, std::string &keyId = _dummy); 	
    std::string getCipherText(CipherKey key, const char *plaintext, bool bChainedNameIV);
    std::string getPlainText(CipherKey fileKey, const char *ciphertext, bool bChainedNameIV); 
...
private:
    ...
    shared_ptr<FileNode> _findOrCreate_safe_reentrant( const char *plainName);    
