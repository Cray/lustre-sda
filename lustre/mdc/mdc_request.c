...
static int mdc_changelog_send_thread(void *csdata)
{
        ...
        if (ctxt == NULL)
                GOTO(out, rc = -ENOENT);
        rc = llog_create(ctxt, &llh, NULL, CHANGELOG_CATALOG, LLOG_CREATE_RW);
        if (rc) {
                          ...
                          renew_capa_cb_t cb)
{
        ...
        req = ptlrpc_request_alloc_pack(class_exp2cliimp(exp), mdc_select_rq_format(exp, RQF_MDS_GETATTR),
