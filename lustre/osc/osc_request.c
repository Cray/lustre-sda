                       ...
                       struct obd_info *oinfo, struct obd_trans_info *oti)
{
	...
	char		      *seclabel = exp->exp_obd->u.cli.cl_se_label;
	ENTRY;
	...
	if (selinux) {
		oinfo->oi_oa->o_valid |= OBD_MD_FLSECURITY;
		  ...
		  struct list_head *ext_list, int cmd)
{
	...
	struct ost_body			*body;
	int				selinux;
	char				*seclabel = cli->cl_se_label;
	ENTRY;
	LASSERT(!list_empty(ext_list));

	selinux = !!(cli->cl_import->imp_connect_data.ocd_connect_flags &
                              ...
                              void *val, struct ptlrpc_request_set *set)
{
	...
	if (KEY_IS(KEY_SEDOMAIN)) {
		strcpy(exp->exp_obd->u.cli.cl_se_domain, val);
		RETURN(0);
	}
	if (KEY_IS(KEY_SELABEL)) {
		strcpy(exp->exp_obd->u.cli.cl_se_label, val);
		RETURN(0);
	}
	if (KEY_IS(KEY_SEMOUNT)) {
		strcpy(exp->exp_obd->u.cli.cl_se_mount, val);
		RETURN(0);
	}
