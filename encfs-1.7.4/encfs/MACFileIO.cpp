...
CipherKey MACFileIO::getFileKey()
{
    key = base->getFileKey();
    return key; 
}

FileID_t MACFileIO::getFileID( std::string &fileIdentifier )
{
    return base->getFileID( fileIdentifier );
}

#  ifdef XATTR_ADD_OPT
int MACFileIO::setxattr( const char *name, const char *value,
              size_t size, int flags, uint32_t position, int options)
{
    return base->setxattr(name, value, size, position, options);
}

ssize_t MACFileIO::getxattr( const char *name, char *value,
              size_t size, uint32_t position, int options ) const
{
    return base->getxattr(name, value, size, position, options);
}

ssize_t MACFileIO::listxattr( char *list, size_t size, int options ) const
{
    return base->listxattr(list, size, options);
}

int MACFileIO::removexattr( const char *name, int options )
{
...
ssize_t MACFileIO::listxattr( char *list, size_t size ) const
{
    return base->listxattr(list, size);
}

int MACFileIO::removexattr( const char *name )
{
