...
int StreamNameIO::encodeName( const char *plaintextName, int length,
	uint64_t *iv, char *encodedName, const CipherKey &fileKey ) const
{
    uint64_t tmpIV = 0;
    if( iv && _interface >= 2 )
	tmpIV = *iv;
    ...
    _cipher->nameEncode( encodeBegin, length, (uint64_t)mac ^ tmpIV, fileKey);
...
int StreamNameIO::decodeName( const char *encodedName, int length,
	uint64_t *iv, char *plaintextName, const CipherKey &fileKey ) const
{
    rAssert(length > 2);
