...
CipherKey SSL_Cipher::newRandomKey()
{
    ...
    //printf("DEBUG:: inside newRandomKey() \n");
    //printf("\n1 key->buffer ::%s",key->buffer);		    
    //printf("\n1 key->keySize %u\n",key->keySize);	
    //printf("\n1 key->ivLength %u\n",key->ivLength);	
	...
	const CipherKey &masterKey)
{
    ...
    //printf("Debug:: Stream key encoded being done:: SSL_Cipher::writeKey\n");	
    //printf("Volume key %s and master key %s",key->buffer+key->keySize,mk->buffer+mk->keySize);
...
void SSL_Cipher::encodedReadKey(const unsigned char *data, unsigned char *encReadKey, 
	const CipherKey &masterKey, bool checkKey)
{
    shared_ptr<SSLKey> mk = dynamic_pointer_cast<SSLKey>(masterKey);
    rAssert(mk->keySize == _keySize);
    
    unsigned char tmpBuf[ MAX_KEYLENGTH + MAX_IVLENGTH + KEY_CHECKSUM_BYTES ] = {0};
    memset( tmpBuf, 0, sizeof(tmpBuf) );
    ...
    memcpy( tmpBuf, data+KEY_CHECKSUM_BYTES, _keySize + _ivLength + KEY_CHECKSUM_BYTES );
    streamDecode(tmpBuf, _keySize + _ivLength + KEY_CHECKSUM_BYTES, checksum, masterKey);

    // check for success
    unsigned int checksum2 = MAC_32( tmpBuf, _keySize + _ivLength + KEY_CHECKSUM_BYTES, masterKey );
    if(checksum2 != checksum && checkKey)
    {
	rDebug("checksum mismatch: expected %u, got %u", checksum, checksum2);
	rDebug("on decode of %i bytes", _keySize + _ivLength + KEY_CHECKSUM_BYTES);
	memset( tmpBuf, 0, sizeof(tmpBuf) );
	return;
    }
    
    memcpy( encReadKey, tmpBuf, _keySize + _ivLength + KEY_CHECKSUM_BYTES );

}

void SSL_Cipher::encodeWriteKey(const unsigned char *encWriteKey, unsigned char *data, 
	const CipherKey &masterKey)
{
    ...
    int bufLen = _keySize + _ivLength + KEY_CHECKSUM_BYTES;
    memcpy( tmpBuf, encWriteKey, bufLen );

    unsigned int checksum = MAC_32( tmpBuf, bufLen, masterKey );
    //printf("enc write key %s and checksum %u",encWriteKey, checksum);
    streamEncode(tmpBuf, bufLen, checksum, masterKey);
    memcpy( data+KEY_CHECKSUM_BYTES, tmpBuf, bufLen );
...
int SSL_Cipher::reEncodedKeySize() const
{
    return _keySize + _ivLength + KEY_CHECKSUM_BYTES + KEY_CHECKSUM_BYTES;
}
...
int SSL_Cipher::encryptData(const CipherKey &ckey, unsigned char *_cipher_out,
                    const unsigned char *_plain_in, int _plain_in_len) 
{
	int dstLen=0, tmpLen=0;
	shared_ptr<SSLKey> key = dynamic_pointer_cast<SSLKey>(ckey);

	EVP_EncryptInit_ex(&key->stream_enc, NULL, NULL, NULL, NULL);	
	EVP_EncryptUpdate(&key->stream_enc, _cipher_out, &dstLen , _plain_in, _plain_in_len);	
	EVP_EncryptFinal_ex( &key->stream_enc, _cipher_out + dstLen, &tmpLen );
...
int SSL_Cipher::decryptData(const CipherKey &ckey, unsigned char *_plain_out,
                           const unsigned char *_cipher_in, int _cipher_in_len) 
{
	int dstLen=0, tmpLen=0;
        shared_ptr<SSLKey> key = dynamic_pointer_cast<SSLKey>(ckey);
        
	EVP_DecryptInit_ex(&key->stream_enc, NULL, NULL, NULL, NULL);
        EVP_DecryptUpdate(&key->stream_enc, _plain_out, &dstLen , _cipher_in, _cipher_in_len);
        EVP_DecryptFinal_ex( &key->stream_enc, _plain_out + dstLen, &tmpLen );
