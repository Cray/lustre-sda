...
static cfs_lock_class_key_t client_key[CFS_LOCK_CLASS_MAX];
...
int client_obd_setup(struct obd_device *obddev, struct lustre_cfg *lcfg)
{
	...
	cfs_init_rwsem(&cli->cl_sem);
		...
		cfs_lockdep_set_class(&cli->cl_sem, &client_key[CFS_LOCK_CLASS_OSC]);
		...
		cfs_lockdep_set_class(&cli->cl_sem, &client_key[CFS_LOCK_CLASS_MDC]);
		...
		cfs_lockdep_set_class(&cli->cl_sem, &client_key[CFS_LOCK_CLASS_MGC]);

        } else {
		cfs_fini_rwsem(&cli->cl_sem);
        ...
        if (cli->cl_dirty_max >> CFS_PAGE_SHIFT > totalram_pages / 8)
                cli->cl_dirty_max = totalram_pages << (CFS_PAGE_SHIFT - 3);
        CFS_INIT_LIST_HEAD(&cli->cl_cache_waiters);
	CFS_INIT_LIST_HEAD(&cli->cl_writepages_waiters);
	...
	cli->cl_max_pages_per_rpc = min_t(int, PTLRPC_MAX_BRW_PAGES,
        ...
        } else if (totalram_pages >> (20 - CFS_PAGE_SHIFT) <= 128 /* MB */) {
                cli->cl_max_rpcs_in_flight = 2;
        } else if (totalram_pages >> (20 - CFS_PAGE_SHIFT) <= 256 /* MB */) {
                cli->cl_max_rpcs_in_flight = 3;
        } else if (totalram_pages >> (20 - CFS_PAGE_SHIFT) <= 512 /* MB */) {
...
out:
	...
	if (target) {
		...
		class_decref(target, __func__, cfs_current());
	}
        if (rc)
                req->rq_status = rc;
        RETURN(rc);
