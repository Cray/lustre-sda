			...
			const struct md_attr *attr)
{
	...
	rc = cfs_security_setattr(uc->uc_sid, osid,  mda.ma_attr.la_mode, &iattr, name);
	if(rc) {
			 ...
			 struct lu_buf *buf, const char *name)
{
	...
	snprintf(file_name, sizeof(file_name), DFID, PFID(sec2fid(md2sec_obj(mo))));
			 ...
			 struct lustre_handle *lockh)
{
	...
	/*TDB: add fsid */
	rc = cfs_security_lock(uc->uc_sid, osid, 0, mda.ma_attr.la_mode, file_name);
	if(rc)
		RETURN(rc);
		      ...
		      struct md_attr *ma)
{
	...
	snprintf(dir_name, sizeof(dir_name), DFID, PFID(sec2fid(md2sec_obj(mo_p))));
	...
	/* Note that the first argument is not uc->uc_sbsid, but rather mntpoint_sid */
	...
	rc = cfs_string_to_sid_default(&new_sid, uc->uc_defsid, attr_value, attr_len - 1);
	if(rc) {
		    ...
		    struct md_attr *ma)
{
	...
	rc = cfs_security_link(uc->uc_sid, dsid, isid, mda.ma_attr.la_mode, pname);
	if (rc < 0) {
		      ...
		      struct md_attr *ma)
{
	...
	if (rc < 0) {
		CDEBUG(D_OTHER, "security_unlink failed for dir %s, object %s, tsid %d, "
				"rc %d\n", pname, cname, uc->uc_sid, rc);
		RETURN(rc);
	}
		      ...
		      const struct lu_name *lt_name, struct md_attr *ma)
{
	...
	struct lu_ucred *uc = lu_ucred(env);
	int same_dirs = lu_object_fid(&mo_po->mo_lu) == lu_object_fid(&mo_pn->mo_lu);
	...
	snprintf(poname, sizeof(poname), DFID, PFID(sec2fid(md2sec_obj(mo_po))));
	snprintf(pnname, sizeof(pnname), DFID, PFID(sec2fid(md2sec_obj(mo_pn))));
	...
	rc = cfs_security_rename(uc->uc_sid, fname, fname,
				 ...
				 same_dirs, !!mo_t);
	if (rc < 0) {
		CDEBUG(D_OTHER, "security_rename failed for olddir %s, newdir %s,"
				" object %s, tsid %d, rc %d\n", poname, pnname,
				fname, uc->uc_sid, rc);
		GOTO(out, rc);
	}
