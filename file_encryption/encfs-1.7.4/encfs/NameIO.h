...
public:
    ...
    std::string decodeDirEntry(const CipherKey &fileKey, const char *encodedDirEntry, uint64_t *iv ) const;
    std::string encodeDirEntry(const CipherKey &fileKey, const char *plainDirEntry, uint64_t *iv ) const;

    CipherKey  getFileKey(const char *plaintextPath,  bool isDirectory, std::string &keyId) const;  
...
protected:	                    
    virtual int encodeName( const char *plaintextName, int length,
	                    char *encodedName, const CipherKey &fileKey) const;
    virtual int decodeName( const char *encodedName, int length,
	                    char *plaintextName, const CipherKey &fileKey ) const;

    virtual int encodeName( const char *plaintextName, int length,
	                    uint64_t *iv, char *encodedName, const CipherKey &fileKey ) const =0;
    virtual int decodeName( const char *encodedName, int length,
	                    uint64_t *iv, char *plaintextName,const CipherKey &fileKey ) const =0;
...
private:
    ...
    std::string recodePath( const char *path,
		...
		uint64_t *, char *, const CipherKey &) const,
	    uint64_t *iv ) const;
