...
std::string DirTraverse::nextPlaintextName(const CipherKey &fileKey, bool bChainedNameIV, int *fileType, ino_t *inode)
{
    struct dirent *de=0;
	...
	try
	{
	    ...
	    return naming->decodeDirEntry(fileKey, de->d_name, iv );
...
bool RenameOp::apply()
{
    try
    {
	std::vector<string> srcDirList; //to be remove...
        std::vector<string> dstDirList; //dst directories are already created, you need to update a_time, m_time on it..
	    ...
	    if(isDirectory(last->oldCName.c_str())) {
		srcDirList.push_back(last->oldCName);
		dstDirList.push_back(last->newCName);
		++last;
		continue;
	    }
	...
	std::vector<std::string> :: iterator srcIter,dstIter;
	for(srcIter = srcDirList.begin(), dstIter = dstDirList.begin(); srcIter != srcDirList.end(); srcIter++, dstIter++) {
		struct stat st;
		string srcDir = (*srcIter);
		string dstDir = (*dstIter);
		bool preserve_mtime = ::stat(srcDir.c_str(), &st) == 0;
		if(preserve_mtime) {
			struct utimbuf ut;
			ut.actime = st.st_atime;
			ut.modtime = st.st_mtime;
			::utime(dstDir.c_str(), &ut);
		}
		if( ::rmdir(srcDir.c_str()) < 0 ) {
			throw ERROR("Error in RenameOp::apply");	
		}
	}	
	...
	const char *fromP, const char *toP )
{
    ...
    string destPath = rootDir + toCPart;
    
    //Create the directory now , as we need key to name its enteries	
    mode_t  mode = getDirectoryMode(sourcePath.c_str());
    if(::mkdir(destPath.c_str(), mode) < 0) 
    {
	throw ERROR("Error occured in DirNode::genRenameList");
    }

    CipherKey srcFileKey = getFileKey(fromP, true); 
    CipherKey dstFileKey = getFileKey(toP, true);	
	...
	try
	{
	    plainName = naming->decodeDirEntry(srcFileKey, de->d_name, &localIV );
	} catch( rlog::Error &ex )
	{
	...
	try
	{
	    ...
	    localIV = toIV;
	    string newName = naming->encodeDirEntry(dstFileKey, plainName.c_str(), &localIV );
	
    	    // store rename information..
    	    string oldFull = sourcePath + '/' + de->d_name;
    	    string newFull = destPath + '/' + newName;
...
DirNode::rename( const char *fromPlaintext, const char *toPlaintext )
{
	...
	if(hasDirectoryNameDependency() &&  isDirectory(fromCName.c_str())) {
		//you have to remove directory, As destination directory is already created..
		res = ::rmdir(fromCName.c_str());
	}
	else {
...
shared_ptr<FileNode> DirNode::_findOrCreate_safe_reentrant( const char *plainName)
{
	//lustrechange
	CipherKey perFileKey;
    //lustrechange	
	...
	int *result )
{
	...
	shared_ptr<FileNode> 
DirNode::_openNode_safe_reentrant( const char *plainName, const char * requestor, int flags,
        int *result )
{
	(void)requestor;
	rAssert( result != NULL );
	
	shared_ptr<FileNode> node = _findOrCreate_safe_reentrant( plainName );
...
CipherKey DirNode::getFileKey(const char *plaintextPath, bool isDirectory, std::string &keyId)
{

        return naming->getFileKey(plaintextPath, isDirectory, keyId);
}


std::string DirNode::getCipherText(CipherKey key, const char *plaintext, bool bChainedNameIV)
{
	uint64_t localIv;
	uint64_t *iv = &localIv;
	if(!bChainedNameIV) {
	...
	return naming->encodeDirEntry(key, plaintext, iv ); //reusing encodeDirEntry() method only for this ....
}


std::string DirNode::getPlainText(CipherKey key, const char *ciphertext, bool bChainedNameIV)
{
	uint64_t localIv;
	uint64_t *iv = &localIv;
	if(!bChainedNameIV) {
	...
	return naming->decodeDirEntry(key, ciphertext, iv ); // reusing decodeDirEntry() method only for this .... 
}



string DirNode :: _dummy;
