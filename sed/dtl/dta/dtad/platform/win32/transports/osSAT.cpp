...
#if defined(USB_RW10)
      // nvn20120206
      // temporary hack to handle read DMA on OLD USB Bridge Firmware
      if (cdb[9] == 0xC8) // Read DMA
      {
         cdb[0] = 0x28;
         cdb[8] = cdb[4]; // low len
         cdb[4] = cdb[6]; // high LBA
         cdb[5] = cdb[7]; // low LBA
         cdb[7] = cdb[3]; // high len         
         // reset the rest
         cdb[1] = 0x00;
         cdb[2] = 0x00;
         cdb[3] = 0x00;
   ...
   )
{
   M_DriveTrustBaseTry()
   {
      ...
      {
         value = m_useAtaPassThrough
         ?  ata::CAta::GetVendor()      // jls 20120816  TXT("")
         :  dta::CScsi::GetVendor()
         ;
      }
      else
