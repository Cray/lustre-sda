...
#ifdef HAVE_XATTR
// return NULL pointer
CipherKey RawFileIO::getFileKey()
{
    return CipherKey();
}

#define MAX_FILE_ID (32)
// For now FileID would be inode no. on lustre need to fetch it aptly
FileID_t RawFileIO::getFileID( std::string &fileIdentifier )
{
    struct stat stbuf;
    int res = stat( name.c_str(), &stbuf );
    int eno = errno;

    if(res < 0) {
	rWarning("stat error on %s: %s", name.c_str(), strerror( eno ));
    }
    else {
      char fileID[MAX_FILE_ID + 1] = {'0'};
      m_fileId = stbuf.st_ino;
      sprintf( fileID, "%032lx", m_fileId );
      fileIdentifier.assign( fileID );  
    }
    return m_fileId;
}

#  ifdef XATTR_ADD_OPT
int RawFileIO::setxattr( const char *attrName, const char *value,
              size_t size, int flags, uint32_t position, int options)
{
    return ::setxattr(name.c_str(), attrName, value, size, position, options);
}

ssize_t RawFileIO::getxattr( const char *attrName, char *value,
              size_t size, uint32_t position, int options ) const
{
    return ::getxattr(name.c_str(), attrName, value, size, position, options);
}

ssize_t RawFileIO::listxattr( char *list, size_t size, int options ) const
{
    return ::listxattr(name.c_str(), list, size, options);
}

int RawFileIO::removexattr( const char *attrName, int options )
{
    return ::removexattr(name.c_str(), attrName, options);
}
#  else
int RawFileIO::setxattr( const char *attrName, const char *value,
              size_t size, int flags)
{
    printf("setxattr(%s, %s, %s, %lu, %d )\n",name.c_str(), attrName, value, size, flags);
    return ::setxattr(name.c_str(), attrName, value, size, flags);
}

ssize_t RawFileIO::getxattr( const char *attrName, char *value,
              size_t size ) const
{
    printf("getxattr(%s, %s, %s, %lu)\n",name.c_str(), attrName, value, size);
    return ::getxattr(name.c_str(), attrName, value, size);
}

ssize_t RawFileIO::listxattr( char *list, size_t size ) const
{
    return ::listxattr(name.c_str(), list, size);
}

int RawFileIO::removexattr( const char *attrName )
{
    return ::removexattr(name.c_str(), attrName);
}
#  endif  //XATTR_ADD_OPT
