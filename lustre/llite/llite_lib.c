	...
	if (data->ocd_ibits_known & MDS_INODELOCK_XATTR) {
        ...
        data->ocd_connect_flags = OBD_CONNECT_GRANT     | OBD_CONNECT_VERSION  |
				  OBD_CONNECT_REQPORTAL | OBD_CONNECT_BRW_SIZE |
                                  ...
                                  OBD_CONNECT_FULL20 | OBD_CONNECT_64BITHASH |
                                  OBD_CONNECT_MAXBYTES | OBD_CONNECT_SHORTIO |
                                  OBD_CONNECT_EINPROGRESS | OBD_CONNECT_SELUSTRE;
...
void ll_lli_init(struct ll_inode_info *lli)
{
        ...
        lli->lli_smd = NULL;
        lli->lli_clob = NULL;
	cfs_init_rwsem(&lli->lli_xattrs_list_rwsem);
	cfs_mutex_init(&lli->lli_xattrs_enq_lock);
                ...
                if (exp_connect_som(ll_i2mdexp(inode)) &&
                    S_ISREG(inode->i_mode) && lli->lli_smd) {
                        ...
                        mode = ll_take_md_lock(inode, MDS_INODELOCK_UPDATE,
                                               &lockh, LCK_CR|LCK_CW|LCK_PR|LCK_PW);
                        if (mode) {
