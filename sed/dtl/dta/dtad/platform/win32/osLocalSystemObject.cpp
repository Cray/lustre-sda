...
#include "setupdi/GetClassDevs.hpp"
...
//#define BUILD_FOR_STORPORT    // 01-20-2012 jls for Win8
      ...
      )
{
   M_DriveTrustBaseTry()
   {
      ...
      if ( logFile.size() )
      {
         m_log.Open( logFile, TXT("Find SED Identifiers") );
      }
      else
      {
         m_log.Close();
         ...
         )
      {
         ...
         // Attempt query and then reallocate for spdidd as necessary.
         ...
         // Device bus type recognized, add a log entry.
         AddLogEntry( dta::Success, sstr.str() );
         ...
         {
            // Add this device to the list of identifiers being returned to caller.
            currentIdentifier += DevicePath;
            identifiers.push_back( currentIdentifier );
         }
#if 0
     /*  // Our first attempt at discovering raid drives
         if (currentIdentifier == _T("RAID:") )
		   {
            char szDrive[16];
            for (i = 0; i < 8; i++)
     ...
     */
         ...
         else  // if (currentIdentifier == _T("RAID:") )
         {
            ...
            for (i = 0; i < 8; i++)
            {
               ...
               {   
                  // Add this device to the identifier list being returned to caller.
                  currentIdentifier += file;
                  identifiers.push_back( currentIdentifier ); 
               }  
            }
         } // if current_identifier != "RAID"

         AddLogEntry( dta::Success, 
            TXT("Added device: '") + currentIdentifier + TXT("' to list.")
            ); 

      } // for each EnumDeviceInterfaces
...
#ifdef BUILD_SEAGATE_INTERNAL_SDK
      // This Code is for the old Win7 Password Silo that is not in Win8.
      ...
      // Log the completion of finding possible SED drives
      AddLogEntry( dta::Success, 
         TXT("Search for possible SED Identifiers is complete.") ); 

   }
   M_DriveTrustBaseSimpleEndTry()

} // GetDriveTrustIdentifiers
...
#ifdef BUILD_FOR_STORPORT
         /// Added for support of NVIDIA and AMD StorPort drivers; 
         /// QueryDeviceParams returns bus type ATA, but since
         /// StorPort treats all drives as SCSI and uses only SCSI
         /// IOCTLs, must create a SAT session so drive responds 
         /// like a SAT device. NOTE: We should threat the identifier
         /// as an opaque string, but in this case we need to see
         /// if the drive is treated as scsi or ide, so parse the
         /// first 3 chars after the "\\?\" header. This will break
         /// at some time in the future, but nopefully not for many
         /// years. [jls 01/20/2012]
//         const _tstring devType = identifier.substr( pos+4, 3 );

         if ( _tstring( identifier.substr( pos+4, 4 )) == _T("scsi") )
         {
            newSession = new COSDTSessionSAT( true ); // Use ATA Passthru
            AddLogEntry( dta::Success,
               TXT("Created new SAT Session object using ATA PassThrough")
            );

            // We also need to change some internal stuff.
            newSession->SetAttribute( TXT("Transport"), TXT("SCSI") );
         }
         else // string not "scsi"

#endif // BUILD_FOR_STORPORT
         {	  
      ...
      else if ( busType == dta::BUS_TYPE_SCSI )
      {
         ...
         AddLogEntry( dta::Success,
            TXT("Created new SCSI Session object using SCSI Passthrough")
            );
      }
      else if ( busType == dta::BUS_TYPE_1394 )
      {
         // Create a session object for SATA PASS-THROUGH
         newSession = new COSDTSessionSAT( true );
         AddLogEntry( dta::Success,
            TXT("Created new SCSI Session object using ATA Passthrough")
            );
      }
      ...
      // Finally, we can create the session object!
