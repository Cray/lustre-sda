...
mode_t getDirectoryMode( const char *fileName )
{
    struct stat buf;
    if( !lstat( fileName, &buf ))
    {
        ...
        const shared_ptr<EncFS_Opts> &opts )
{
    ...
    //POC TODO get config type from user default ?!
    char answer[10] = {0};
...
{	
	...
	printf("Debug::Expecting this path:: cipher->newKey\n");
        userKey = cipher->newKey( password, passwdLen,
                kdfIterations, desiredKDFDuration, 
                getSaltData(), salt.size());
    } else
    {
        printf("Debug::Expecting this path:: else cipher->newKey\n");
	userKey = cipher->newKey( password, passwdLen );
    }
