...
int __attribute__((weak)) avc_has_perm(u32 ssid, u32 tsid, u16 tclass,
					u32 requested,
					struct common_audit_data *auditdata);

extern __attribute__((weak)) int selinux_policycap_openperm;
int __attribute__((weak)) security_validate_transition(u32 oldsid, u32 newsid,
					u32 tasksid, u16 tclass);
int __attribute__((weak)) security_sid_to_context_force(u32 sid, char **scontext,
					u32 *scontext_len);
int __attribute__((weak)) security_context_to_sid_default(const char *scontext,
					u32 scontext_len,
					u32 *out_sid, u32 def_sid,
					gfp_t gfp_flags);
int __attribute__((weak)) security_context_to_sid_force(const char *scontext,
					u32 scontext_len,
					u32 *sid);
int __attribute__((weak)) security_transition_sid(u32 ssid, u32 tsid,
					u16 tclass, u32 *out_sid);
void __attribute__((weak)) security_inode_post_setxattr(struct dentry *dentry,
...
{
	if (&selinux_policycap_openperm == NULL)
		return 0;
...
/**
 * \brief Get SID of the current thread.
...
int cfs_sid_to_string(__u32 sid, char *str)
{
	...
	rc = security_secid_to_secctx(sid, &s, &len);
	if (rc == 0) {
		memcpy(str, s, len);
		str[len] = 0;
		security_release_secctx(s, len);
	} else {
...
int cfs_string_to_sid(__u32 *sid, char *str, int len)
{
	...
	rc = security_secctx_to_secid(str, len, sid);
	if (rc != 0)
		printk(KERN_WARNING "unable to convert %.*s to SID, rc=%d\n",
		       len, str, rc);
...
int cfs_string_to_sid_default(__u32 *sid, __u32 def_sid, char *str, int len)
{
	...
	if (security_context_to_sid_default == NULL)
		return -ENOSYS;
...
int cfs_security_from_inode(struct inode *inode, char *seclabel)
{
	...
	rc = xattr_getsecurity(inode, "selinux", seclabel, CFS_SID_MAX_LEN);
	if (rc > 0)
		seclabel[rc] = '\0';
...
int cfs_sid_cache_init(cfs_sid_cache_t **cache)
{
	...
	*cache = (cfs_sid_cache_t *)kmalloc(sizeof(**cache), GFP_KERNEL);
	if (*cache == NULL)
		return -ENOMEM;
		      ...
		      u16 tclass, const char *name)
{
	...
	if (avc_has_perm == NULL || security_transition_sid == NULL)
		return -ENOSYS;
	...
	if (!tcsid) { /* Assume SE_SBLABELSUPP */
			       ...
			       const char *value, int size, int flags)
{
	...
	if (security_inode_post_setxattr == NULL)
		return -ENOSYS;
			  ...
			  __u64 capa, __u32 fsuid, __u32 iuid)
{
	...
	if (avc_has_perm == NULL ||
	    security_validate_transition == NULL ||
	    security_context_to_sid_force == NULL)
		return -ENOSYS;
	...
	rc = security_secctx_to_secid(value, size, &newsid);
	if (rc == -EINVAL) {
			     ...
			     size_t *len)
{
	...
	if (security_sid_to_context_force == NULL ||
	    security_transition_sid == NULL)
		return -ENOSYS;
