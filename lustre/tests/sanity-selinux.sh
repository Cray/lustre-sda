...
$LCTL dl | grep mdt && error "client node should not be running MDT for this test"
...
test_1() {
	...
	runcon -l s0 multiop $DIR/$tfile N $DIR/${tfile}2 && echo "managed to rename"
	runcon -l s0 getfattr -n user.attr $DIR/$tfile && echo "managed to getxattr"
	runcon -l s0 setfattr -n user.attr -v val $DIR/$tfile && echo "managed to setxattr"
