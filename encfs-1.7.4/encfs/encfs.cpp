...
int encfs_getdir(const char *path, fuse_dirh_t h, fuse_dirfil_t filler)
{
	...
	std::string keyId;
        CipherKey fileKey = FSRoot->getFileKey(path, true, keyId);
	...
	if(dt.valid())
	{
	    ...
	    std::string name = dt.nextPlaintextName(fileKey, FSRoot->hasDirectoryNameDependency(), &fileType, &inode );
	    while( !name.empty() )
	    {
		...
		name = dt.nextPlaintextName(fileKey, FSRoot->hasDirectoryNameDependency(), &fileType, &inode );
	    } 
	} else
	{
...
int _do_readlink(EncFS_Context *ctx, const string &cyName,
	tuple<char *, char *, size_t> data )
{
    char *path = data.get<0>();
    char *buf = data.get<1>();
    size_t size = data.get<2>();
    ...
    try
    {
	CipherKey fileKey = FSRoot->getFileKey(path, false);
	//decodedName = FSRoot->plainPath( buf );  //lustre change
	decodedName = FSRoot->getPlainText(fileKey, buf, FSRoot->hasDirectoryNameDependency());	
    } catch(...) { }
...
int encfs_symlink(const char *from, const char *to)
{
	...
	//string fromCName = FSRoot->relativeCipherPath( from ); //lustre change..
	CipherKey fileKey = FSRoot->getFileKey(to, false);
	string fromCName = FSRoot->getCipherText(fileKey, from, FSRoot->hasDirectoryNameDependency());
...
int _encfs_open_safe_reentrant(const char *path, struct fuse_file_info *file)
{
    EncFS_Context *ctx = context();
    ...
    {
	shared_ptr<FileNode> fnode = 
	    FSRoot->_openNode_safe_reentrant( path, "open", file->flags, &res );
