		...
		const char *plaintextName_, const char *cipherName_)
{
    ...
    this->fsConfig = cfg;
    //lustre change
    //this->_perFileKey = cfg->config->getFileKey();
...
#ifdef HAVE_XATTR
    // create file key and store in Xattr in encoded form or fetch the file key from Xattr
    io->getFileKey();
#endif //HAVE_XATTR       


    rDebug("    Constructor called for filename  %s \n", _pname.c_str());
}
...
FileNode::~FileNode()
{
    ...
    rDebug("    Destructor called for filename  %s \n", _pname.c_str());
...
CipherKey FileNode::getFileKey() const
{
	return io->getFileKey();
}


FileID_t FileNode::getFileID(std::string &id) const
{
	return io->getFileID(id);
}
